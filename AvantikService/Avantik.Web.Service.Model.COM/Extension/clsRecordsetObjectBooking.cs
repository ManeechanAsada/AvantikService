            ...
            if (passenger != null && passenger.Count > 0)
            {
                try
                {
                    foreach (Passenger p in passenger)
                    {
                        ...
                        rs.Fields["known_traveler_number"].Value = p.KnownTravelerNumber;

                        //EDWDEV-81 
                        rs.Fields["pnr_firstname"].Value = p.Firstname;
                        rs.Fields["pnr_lastname"].Value = p.Lastname;

                        if (!string.IsNullOrEmpty(p.Middlename))
                        {
                            if (!string.IsNullOrEmpty(p.TitleRcd))
                                rs.Fields["pnr_pax_name"].Value = p.Lastname + "/" + p.Firstname + " " + p.Middlename + p.TitleRcd;
                            else
                                rs.Fields["pnr_pax_name"].Value = p.Lastname + "/" + p.Firstname + " " + p.Middlename;
                        }
                        else
                        {
                            if (!string.IsNullOrEmpty(p.TitleRcd))
                                rs.Fields["pnr_pax_name"].Value = p.Lastname + "/" + p.Firstname + p.TitleRcd;
                            else
                                rs.Fields["pnr_pax_name"].Value = p.Lastname + "/" + p.Firstname;
                        }
            ...
            if (service != null && service.Count > 0)
            {
                try
                {
                    foreach (PassengerService s in service)
                    {
                        ...
                        rs.Fields["special_service_status_rcd"].Value = s.SpecialServiceStatusRcd;
